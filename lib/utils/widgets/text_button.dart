...
class NTextButton extends StatelessWidget {
  ...
  /// The background color of the button. Typically transparent for text buttons,
  /// but can be set for hover/focus effects.
  ...
  /// The shape of the button, including border radius. Affects the splash area.
  ...
  Widget build(BuildContext context) {
    ...
    final ButtonStyle style = ButtonStyle(
      ...
      backgroundColor: WidgetStateProperty.resolveWith<Color?>(
          ...
          // Optional: Add subtle background on press/focus if desired
          ...
          return effectiveBackgroundColor; // Normal state background
        },
      ),
      ...
      // Overlay color for ripple effect (usually based on foreground)
      ...
      // Elevation (typically 0 for text buttons)
      elevation: WidgetStateProperty.all<double>(0.0),
      // Padding
      padding: WidgetStateProperty.all<EdgeInsetsGeometry?>(padding ?? const EdgeInsets.symmetric(horizontal: 8.0, vertical: 10.0)), // Adjusted padding
      // Shape and Border Radius (affects splash shape)
      shape: WidgetStateProperty.all<OutlinedBorder>(
        shape ?? RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(8.0), // Default border radius for splash
        ),
      ),
